...
allocproc(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->state == UNUSED){
      //for(ap = procTable.proc; ap < &procTable.proc[NPROC]; ap++){
        //if(ap->state == UNUSED){
          goto found;
        //}
      //}
...
found:
  ...
  //p->proc = ap;
  p->proc = procTable.proc + (int)(p-ptable.proc);
  p->state = EMBRYO;
  p->proc->pid = nextpid++;
  release(&ptable.lock);
...
void killsiblingthreads() {
  ...
  do{
    killing = 0;
    for(p = ptable.proc; p < & ptable.proc[NPROC]; p++){
      if((p->proc == current->proc)&&(p != current)&&(p != initproc)&&(!p->killed)){ 
        killing = 1;
        p->killed = 1;
        numberOfHits++;
        acquire(&ptable.lock);
...
t_exit(void){
  ...
  current->state = ZAMBIE;
  wakeup1(current->proc);
  //current->proc = &current->temporarilyhere;
  sched();
...
clone(void (*function)(void), char* stack) {
  ...
  nt->tid = (int)(nt);
  //nt->proc->state = ZOMBIE;
  nt->proc = current->proc;
  *nt->tf = *current->tf;
  nt->tf->esp = ((uint)stack)-4;
...
void t_join(int tid){
  struct thread *p = (struct thread*) tid;
  //int haveSibs;
  for(;;){
    if(p->state == ZAMBIE){
      acquire(&ptable.lock);
