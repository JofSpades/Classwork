...
struct inode_functions procfs_i_node = {procfs_ipopulate, fs_iupdate, procfs_readi, fs_writei};
...
procfs_readi(struct inode *ip, char *dst, uint off, uint n){
  lockTable();
  int count = numRunning();
  unlockTable();
  struct inode *temp = namei("/proc");
  temp->size = (2+count)*16;
  if(ip->inum == 1000 && off == 0){
    char meminfo[512];
    meminfo[0] = '\0';
    uint count = 0;
    struct run *r;
    ...
    strcat(meminfo, "Free Bytes: ");
    char free[32];
    itoa(((uint)count/1024), free);
    strcat(meminfo, free);
    strcat(meminfo, " KB.\nTotal Bytes: ");
    char total[32];
    itoa(((uint)fullsize()/1024), total);
    strcat(meminfo, total);
    strcat(meminfo, " KB.\n\0");
    memmove(dst, meminfo, strlen(meminfo));
    return strlen(meminfo);
  } else if(ip->inum == 1001 && off == 0){
    int count = 0, i;
    for(i = 0; i < NCPU; i++){
      if(cpus[i].started){
        count++;
    ...
    strcat(cpuinfo, on);
    strcat(cpuinfo, "\n\0");
    memmove(dst, cpuinfo, strlen(cpuinfo));
    return strlen(cpuinfo);
  } else if (ip->inum > 10000 && off < 32){
    int pid = ip->inum - 10000;
    struct dirent procde[2];
    procde[0].name[0] = '\0';
    procde[1].name[0] = '\0';
    strcat(procde[0].name, "name");
    procde[0].inum = 2000 + pid;
    strcat(procde[1].name, "parent");
    procde[1].inum = 3000 + pid;
    memmove(dst, (char*)(&procde[off/n]), sizeof(procde[off/n]));
    return 16;
  } else if (ip->inum < 1000 && (off/n < 2+count)){
    lockTable();
    int *pids = getPids();
    unlockTable();
    struct dirent procde[2+count];
    procde[0].inum = 1000;
    procde[0].name[0] = '\0';
    strcat(procde[0].name, "meminfo");
    procde[1].inum = 1001;
    procde[1].name[0] = '\0';
    strcat(procde[1].name, "cpuinfo");
    char pid[32];
    int i;
    for(i = 0; i < count; i++){
      int pidn = pids[i];
      procde[i+2].inum = 10000+pidn;
      itoa(pidn, pid);
      procde[i+2].name[0] = '\0';
      strcat(procde[i+2].name, pid);
    }
    memmove(dst, (char*)(&procde[off/n]), sizeof(procde[off/n]));
    return 16;
  } else if((ip->inum > 2000 && ip->inum < 3000) && off == 0){
    int pid = ip->inum - 2000;
    lockTable();
    char name[14];
    memset(name, 0, 14);
    strcat(name, getName(pid));
    strcat(name, "\n\0");
    unlockTable();
    memmove(dst, name, sizeof(name));
    return sizeof(name);
  } else if((ip->inum > 3000 && ip->inum < 10000) && off == 0){
    int pid = ip->inum - 3000;
    char parent[32];
    memset(parent, 0, 32);
    if(pid == 1){
      itoa(1, parent);
      ...
      lockTable();
      itoa(getParPid(pid), parent);
      unlockTable();
    }
    strcat(parent, "\n\0");
    memmove(dst, parent, sizeof(parent));
